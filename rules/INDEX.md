...
| [workflow-documentation](./workflow-documentation.md) | workflow | Documentation management constraints — minimise, DRY, naming for temporary documents. The decision tree lives in docs/guides. | Creating or maintaining a .md document |
...
| [adr-management](./adr-management.md) | workflow | ADR discipline: the two-question admission test, status field enforcement (5-value enum), the decay policy (archive at 12 months, deletable after 6, ADRs themselves exempt from deletion), and the boundary with decisions. | Writing or maintaining an ADR |
...
Rules are injected into the AI runtime as long-lived background context. While any atomic skill executes, the constraints a rule defines take highest precedence.
