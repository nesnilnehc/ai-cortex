...
| [architecture-quality](./architecture-quality.md) | quality | Stable architecture criteria for responsibility ownership, dependency direction, cycles, boundary leakage, contract evolution, coupling, composition, change surface, unreachable code and deprecation discipline. | Reviewing or designing production code with meaningful module or contract boundaries |
...
| [error-surfacing-quality](./error-surfacing-quality.md) | quality | Stable criteria for where a defect is decided and how a failure is reported: boundary decisions on untrusted input, stopping on a broken invariant, detecting at the earliest layer that can decide, and messages an actor — person or program — can act on. | Reviewing code that reads input it does not control, or reports a failure |
| [literal-and-copy-quality](./literal-and-copy-quality.md) | quality | Stable criteria for a value that exists in more than one place: one defining source for a cross-runtime value, display formats from a formatting API, whole copy entries, full language coverage, and fallback copies that agree with their source. | Reviewing a change that produces text a person sees, or a value more than one runtime reads |
...
| [adr-management](./adr-management.md) | workflow | ADR discipline: the two-question admission test, status field enforcement (5-value enum), the decay policy (archive at 12 months, deletable 6 months after that, ADRs themselves exempt from deletion), the boundary with decisions, and which maintenance edits a written record still allows. | Writing or maintaining an ADR |
