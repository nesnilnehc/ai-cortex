...
> **自解释说明（IDE / Agent 必读）**  
> 本文件即「测试入口」：**仅阅读本文件并按 [§ 执行清单](#执行清单agent--ide-可直接执行) 操作，即可完成仓库内所有带测试技能的验证**。数据源与步骤均在下方写明，无需再读其他脚本或各技能目录。
...
- **可执行部分**：执行清单明确「如何跑」全量测试，供 Agent/IDE 直接按步骤运行。  
...
3. **对每个用例**（即 `cases.json` 中的每一条）
   - 以该条的 **`input`** 为输入，**按该技能的 SKILL.md 执行一次**（在 IDE/对话中把 input 交给执行方，并指示其按对应 SKILL.md 执行）。
...
- 安装脚本会按 `test_path` 拉取 `assertions.md`、`cases.json` 到消费方；运行策略由消费方或本仓库贡献流程自行决定，本规范仅约定格式与推荐执行时机。
