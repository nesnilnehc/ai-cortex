...
盘点治理输入源并生成下一步行动路由，**默认 `execute=false` 只输出计划**，不执行下游技能。
...
| 执行 | 默认只规划；`execute=true` 时按推荐顺序串调下游 | 不自动推进——只导航不开车，跑什么 / 何时跑由用户决定 |
...
- 默认 `execute=false` 只产路由建议；`execute=true` 时按推荐顺序串调下游
