...
- **链接约定无需选择**：AI Cortex 的 canonical 约定（按 [artifact-contract §2](../../specs/artifact-contract.md#2-制品类型) 的 slug 命名）已硬编码在所有产出技能 `path_pattern` 里；项目若想用聚合目录 / 父指针 / 清单等非默认约定，通过 `ARTIFACT_NORMS.md` 的 `path_pattern` 覆盖或物理 manifest 文件表达即可，不需要声明"模式"。v8.0 移除 v7.0 引入的 `linking_mode` 枚举字段（见 ADR 005）
...
7. ✅ Now tier 下游扫描按物理信号工作（resolved `path_pattern` + 可选 `parent:` frontmatter + 可选 manifest 文件）；不依赖 `linking_mode` 字段，不触发"先识别模式"的前置闸门
...
`cache` 的主要用途：步骤 2.5 读 `artifact_types[<type>].path_pattern` 决定下游扫描目录；步骤 2.7 S5 扫描按覆盖值扫任务路径（让 colocation 项目能扫到 `work/<slug>/tasks.md`）。**v8.0 起不再读 `linking_mode` 字段**（该字段已退役）。
...
- **（v8.0 移除）**原本 v7.0 此处说明读 `linking_mode` 字段——该字段已退役（ADR 005）。扫描策略统一改为物理信号检测，见 2.5。
...
#### 2.5 Now tier 下游扫描策略（v8.0 简化——无 mode 枚举）

v7.0 曾引入 `linking_mode` 枚举字段协调扫描策略。v8.0 回撤此设计（见 ADR 005），改为**按实际数据工作**：
...
- ❌ **不要引入 `linking_mode` 字段读取或分支逻辑** —— v7.0 曾这样做，v8.0 已回撤（ADR 005）。扫描直接按物理信号（resolved path_pattern + parent: frontmatter + manifest 文件存在）
- ❌ **不要让内部术语泄漏到用户视角**（统一规则）：
...
- [ ] Step 2.5 按物理信号扫描（resolved path_pattern + 可选 parent: + 可选 manifest）；未读 `linking_mode` 字段，未触发"先识别模式"闸门
