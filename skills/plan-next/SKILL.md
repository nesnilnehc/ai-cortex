...
- **链接模式识别结果读取**：按优先级读取——(1) `artifact_norms_path` 里 `define-docs-norms` 写入的 `linking_mode` 字段；(2) `norms_proposal_path` 里 `discover-docs-norms` 上次的识别结果；(3) 都无则记为"未识别"，触发 2.5 的前置闸门路由。**plan-next 不自行识别模式**，交由 `discover-docs-norms` 处理。
...
1. 读 `artifact_norms_path` 的 `linking_mode` 字段（由 `define-docs-norms` 写入，代表用户已选定）
2. 若无，读 `norms_proposal_path` 的识别结果（由 `discover-docs-norms` 上次扫描产生，代表未批准的候选）
3. 若两者都无 / 过期 → **前置闸门路由**：在"现在该做"加一条 `discover-docs-norms` → `define-docs-norms` 作为 Now tier 下游评估的前置动作，优先级高于其他下游缺口路由（因为未识别模式时下游扫描精度不足）

**按识别结果的扫描策略**：

| 读取到的模式 | plan-next 扫描行为 |
|---|---|
| slug | 按 Now tier slug 在各下游目录 glob 匹配 |
| colocation | 按 `work/<slug>/` 目录结构匹配 |
| parent-pointer | 构建 `parent:` 字段反向索引匹配 |
| manifest | 以清单为真相源扫；同时对比物理文件与清单差异作为 G3 漂移报出（plan-next 只读清单不写） |
| mixed | 按 `discover-docs-norms` 返回的多模式组合逐层扫 |
| none | 触发前置闸门路由（同上第 3 点） |
...
- [ ] 已读 `discover-docs-norms` 的识别结果（`ARTIFACT_NORMS.md` 的 `linking_mode` 字段优先，`norms_proposal_path` 兜底）；缺失 / 过期时已路由前置闸门，未自行启发识别
