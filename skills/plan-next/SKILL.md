...
| 2.1 状态识别 | 项目处于哪个治理阶段（7 态 S1-S7）？ | 状态标签 + **聚焦范围**（S5 命中时下钻 2.7 子状态机） |
| 2.3 缺口判定 | 聚焦范围内有哪些缺口（4 类 G1-G4）？ | 缺口列表 → 矩阵路由 |

**协作关系**：状态机决定**看哪儿**（聚焦），缺口矩阵决定**那儿缺什么**（4 类 G1-G4），2.4 决策规则合并优先级 → "现在该做" 列表。

**辅助子步骤**：

- **2.2** —— 2.1 状态识别的 fallback（势均力敌时退化）
- **2.5 / 2.6** —— Now tier 下游扫描的细化（仅当聚焦含 Now tier 时介入）
- **2.7** —— 2.1 中 S5 状态的子状态机（S5 命中时下钻：任务 × 代码 2×2 交叉，产出健康/卡点/3 类 G3 漂移）
