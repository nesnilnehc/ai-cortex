...
> **角色**：治理入口路由器
> **WHAT**：按三步法 **扫**（盘点治理资产）→ **诊**（识别项目所处的治理阶段 7 态 + 资产缺陷 4 类）→ **荐**（产出三节路由报告：现在该做 / 其他可留意 / 诊断依据）
> **HOW**：**只规划不执行**（read-only）；自动化场景由外层 orchestrator + `loop` 组合驱动；单一维度问题直接调专用技能（`assess-docs` / `align-planning` 等）
...
盘点治理输入源并生成下一步行动路由——**read-only**，永不执行下游技能。

- **看全局、决定下一步跑哪个**：是入口路由器
- **不适用时可跳过**：单一维度问题（只查就绪度、只查漂移、已知某项缺失）直接跑专用技能
- **适用时机**：迭代收尾 / 发布前治理路径确认 / 对下一步无头绪时

### 执行态边界

| 维度 | plan-next 做 | plan-next 不做 |
|---|---|---|
| Task 状态 | 读当前状态 × 代码活动交叉**诊断**治理健康（步骤 2.7） | 不记 task 历史；不答"本周晋升几条"类时序问题 |
| 路由 | 输出"现在该做"治理路由建议 | 不充当外部系统的 task 状态 API；不维护 task 列表 / 不分配 |
| 执行 | **只规划，不执行**——产路由建议交用户 / 上层 orchestrator 决定 | 不自动推进；自动化由外层组合（见 [自动化组合](#自动化组合)） |

### 链接约定

AI Cortex canonical 约定（[artifact-contract §2](../../specs/artifact-contract.md#2-制品类型)：requirements / designs / tasks 统一为 `docs/<type>/{slug}.md`）已硬编码在所有产出技能 `path_pattern` 里。项目要用聚合目录 / 父指针 / 清单等非默认约定，通过 `ARTIFACT_NORMS.md` 的 `path_pattern` 覆盖 + 可选 `upstream_ref` 输入 + 物理 manifest 文件表达即可。

### 自动化组合

plan-next 不内含执行能力。自动化迭代由外层技能组合驱动，分三层原语：

| 层 | 职责 | 技能 |
|---|---|---|
| 诊断（read-only） | 盘点资产 + 识别状态/缺口 + 产出路由建议 | **plan-next**（本技能） |
| 驱动（执行 1 步） | 读 plan-next 输出，挑"现在该做"最高优先级 1 条执行；带用户确认 / 失败处理 | orchestrator 技能（如 `auto-iterate`） |
...
- `/loop /auto-iterate 30m` —— 周期诊断 + 自动执行下一步（autopilot）

plan-next 始终是**最内层、纯只读**的诊断器；执行与调度由外层技能负责。
...
**负责**：资产扫描 / 状态诊断 / 缺口识别 / 路由建议产出

**不负责**：

- 默认模式下执行下游技能
- 文档深度评估（→ `assess-docs`）/ 漂移校准（→ `align-planning`）/ 矩阵中推荐技能的实际执行
- 流水线状态追踪（不记忆 todo→in-progress→done 的历史转换）
- 维护下行清单（项目有清单时只读不写，差异作 G3 漂移；维护是 `align-work-item-manifest` 的事）

**Handoff**：路由产出后由用户按优先级执行下游技能；遇阻按"上抛条件"退回 plan-next 重新评估。
...
- **stateless**：每次从零重扫，不依赖上次结果
- **read-only**：只产路由建议，永不执行下游技能

诊断分三步：**扫（Scan）→ 诊（Diagnose）→ 荐（Recommend）**。

### 步骤 0：Norms Resolution

按 [artifact-contract §8.2](../../specs/artifact-contract.md#82-发现顺序) 顺序读项目规范到 `cache`：依序检查 `artifact_norms_path`（输入）→ `.ai-cortex/artifact-norms.yaml` → `docs/ARTIFACT_NORMS.md`，YAML 畸形则 HALT 诊断；缺失则继续不报错。`cache` 用于步骤 2.5（下游扫描路径）和步骤 2.7（任务源路径）的 `path_pattern` 解析。
...
抽象层互斥（意图 / 实施 / 元规则），主题在意图层内细分；细化字段是同主题的辅助维度，**不是独立扫描**，供步骤 2.5 / 2.7 消费。
...
|---|---|
| **路径** | 文件系统路径 |
| **状态** | `present` / `placeholder`（仅含 `[TODO]`/`<待填>`/`TBD`）/ `missing` |
| **质量** | frontmatter `quality` 字段 → 否则按必填字段非空率估 |
| **最后更新** | `git log -1 --format=%cI <path>` → 不可访问时取 fs mtime |

### 步骤 2：诊（Diagnose）— 状态 + 缺口 + 执行健康

**4 子步骤总览**（依次执行，每步可短路或下延）：
...
**协作关系**：状态机决定**看哪儿**，缺口矩阵决定**那儿缺什么**，2.4 决策规则合并优先级 → "现在该做" 列表。2.5 / 2.6 是 Now tier 下游扫描的细化（仅当聚焦含 Now tier 时介入）；2.2 是状态识别的 fallback。

**两套并列的发现类型**（防混淆）：

| 类型 | 范围 | 元素 |
|---|---|---|
| **缺口型 MECE** | 进缺口矩阵（按 G1-G4 分类） | • 2.3 矩阵直接产出：各（主题 × 缺口）单元格<br>• S6 漂移校准 → G3（What/When 或 Is）<br>• S5 的 3 类漂移（追踪 / 完成 / 归档）→ 都是 G3 子类（任务 × 代码两真相源不一致） |
| **工作流动作型** | 不进矩阵，状态机直接路由特定动作 | S4 ceremony（pull）/ S5 健康（静默）/ S5 卡点（`investigate-root-cause` + 人工）/ S7 idle |

**2.7 执行健康与 7 态/4 类的关系**：2.7 是 **S5 的子状态机**（在 7 态内，作为 S5 的细化）；5 类输出中 3 类是 G3 真相漂移子类、2 类是工作流动作型。单列 2.7 的理由：共同信号源（任务 × 代码）+ 共同决策规则（单一 2×2 真值表）+ S5 专属触发前提。
...
**为什么 7 态**：项目治理生命周期的 7 个有意义阶段——S1-S3 治理建立期（上游：愿景 → 路线 → backlog 充实）+ S4-S6 执行动态（下游：停滞 → 执行 → 收尾漂移）+ S7 稳定。不是预设数字，是从实证项目演进路径上识别出的可区分阶段。

状态机消费步骤 1 的扫描结果，产出**状态标签 + 聚焦范围**。状态分两类聚焦形态：缺口型（落矩阵单元格）与工作流动作型（不对应矩阵）。
...
| S5 | **执行中** | 有 in-progress task / 未合并 PR / 近期 commit | 任务 × 代码交叉判（见 2.7）：健康静默 / 卡点路由 |
...
多个候选状态势均力敌（2.4 tiebreak 仍无法择一）→ 退化为全矩阵输出（"其他可留意"展开），诊断依据明示"状态未定"并列各候选支撑信号。
...
1. **多状态 tiebreak**：
   ...
   - **执行维度内**：取下游（S6 > S5 > S4 > S7）——越后期越紧迫（如 S5+S6 取 S6 优先校准漂移）
2. **聚焦决定"现在该做"**：状态机聚焦范围内的发现进"现在该做"，外部进"其他可留意"
...
三层检测**独立可组合**，无需 mode 配置——直接看物理信号。**项目定制**：
...
**概念位置**：本节是 S5 状态的子状态机（在 7 态内）。5 类输出中——3 类漂移（追踪/完成/归档）属 G3 真相漂移子类，2 类（健康/卡点）属工作流动作型。
...
- **Tiebreak**：多个卡点按任务自身 `priority` 排序——但任务 priority **不参与** plan-next 路由 P0-P3（治理层级不让任务僭越）
...
**来源**：消费步骤 2 的产出——状态机聚焦范围 + 缺口矩阵（G1-G4 路由）+ S5 执行健康判定 + 2.4 决策规则（tiebreak / 聚焦 / 排序）。步骤 3 不引入新逻辑，只做**字段填充与呈现**。

**4 条指导原则**：

1. **分层按聚焦**：聚焦范围内 → "现在该做"；范围外 → "其他可留意"；判定过程 → "诊断依据"（详见 [3.3](#33-用户输出三节)）
2. **优先级按治理紧迫性**：越上游越优先（P0 根基 > P1 战略/计划 > P2 设计/实现 > P3 其余）；**不按缺口数量**排（详见 [3.1](#31-优先级治理紧迫性)）
3. **一条路由对一个矩阵格**：不混缺口类型；六字段齐全；矩阵 `—` 单元格不强行推荐技能（详见 [3.2](#32-每条路由六字段)）
4. **深度优先 + 术语隔离**：每个 Now tier 条目一次只报最上游缺口（见 2.6）；前两节用自然语言，内部编号（S/G/P、主题分类词）仅在"诊断依据"以括注追溯
...
**主题写法**：前两节用自然语言描述资产（"路线图"、"战略目标"、"规范文件"），不用作者视角分类词（Why / What/When / How / Is / Rules）。
...
- **永不调用下游技能**（read-only 硬边界）；自动化由外层 orchestrator 负责
- 不隐藏跳过原因（short-circuit 或矩阵 `—` 时必须明示）
- 不混入下游执行细节（不写 ADR、不修代码、不整结构）
- stateless（每次从零重扫）
- 内部术语（S1-S7、G1-G4、P0-P3、主题分类词）使用规则见下方 Anti-Patterns
...
- ❌ 模糊措辞写路由（"可能 / 或许 / 可以考虑"）
- ❌ 省略停止条件（会导致重跑 plan-next 反复推荐同一条）
- ❌ 一条路由混多个缺口类型（一对一对应矩阵单元格）
- ❌ 按缺口数量给优先级（"5 个 G1 比 1 个 G3 重要"——优先级由治理层级决定）

**关于状态机与执行健康**：

- ❌ 用状态机折叠"其他可留意"（状态机是聚焦工具，不是过滤器；外部缺口必须呈现，无发现时写"无"）
- ❌ 状态不明强行归类（低 confidence 时 fallback 全矩阵）
- ❌ 仅凭任务状态或仅凭 commit 单边判执行健康（必须 2×2 交叉，否则会把维护不规范误判为健康）
- ❌ 用任务 priority 决定 plan-next 路由 P0-P3（任务 priority 仅供卡点间 tiebreak）

**关于 Now tier 与扫描**：

- ❌ 对 Next/Later tier 报下游缺口（仅评估 Now tier）
- ❌ 同时报同一条目的多层缺口（深度优先：一次只报最上游）
- ❌ 引入 mode 枚举或配置字段（直接看物理信号）
- ❌ 承担下行清单维护职责（只读，差异作 G3；维护是 `align-work-item-manifest` 的事）

**关于内部术语泄漏**（统一规则）：

- ❌ 前两节使用 S1-S7 / G1-G4 / P0-P3 / 主题分类词；主题列用自然语言（"路线图"、"战略目标"）
- ❌ 诊断依据用 S 编号作主语（应用自然语言，S 编号至多括注追溯如"（S2）"）
- ❌ 示例标题写"S2 → S3 过渡"（用自然语言场景描述）
- ❌ 返回任何执行结果（read-only——只返回路由建议）

### 执行者映射（路由矩阵推荐技能的归类）

DEFINE / ASSESS / ALIGN / ORGANIZE 四类动作的实际执行者已在 §2.3 路由矩阵列出，不再单列。
...
- [ ] 步骤 0 cache 已构建或明示"no norms found"；未静默使用默认
- [ ] 3 抽象层 × 5 主题全覆盖，每项资产 4 字段齐
- [ ] roadmap tier（Now/Next/Later）已读；未分层时已识别为"需先 promote"

**诊**：

- [ ] 状态识别完成；多状态匹配已应用 tiebreak（跨维度 / 治理上游 / 执行下游）；低 confidence 已 fallback
- [ ] 聚焦范围已明示（缺口型 or 工作流动作型）
- [ ] S5 命中时已做任务 × 代码 2×2 交叉，产出健康 / 卡点 / 追踪 / 完成 / 归档之一
- [ ] Now tier 下游扫描按物理信号；Next/Later 未被误报为缺口

**荐**：

- [ ] "现在该做"每条六字段齐
- [ ] 聚焦内按 P0>P1>P2>P3 排序；外进"其他可留意"
- [ ] 矩阵 `—` 未被强行推荐；停止条件三选一齐全
- [ ] 每个 Now tier 条目仅报最上游缺口（深度优先）

**输出**：

- [ ] 前两节仅自然语言；内部编号只在"诊断依据"
- [ ] 未调用任何下游技能（read-only）
- [ ] "其他可留意"超 5 条已折叠；short-circuit 已省略
- [ ] 诊断依据注明扫描信号组合（slug / parent / manifest 检测情况）
...
- **项目情况**：执行收尾阶段；漂移校准已过期
- **判定规则**：近 14 天有 merge/release + 上次 align-planning > 14 天 → 匹配 S6；聚焦 = 漂移校准（工作流动作型）。其他发现归"其他可留意"
