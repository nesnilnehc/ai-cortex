...
**Phase 0.5 (Planning Readiness Gate)**: When project docs are missing, runs `discover-docs-norms`, `bootstrap-docs`, or `assess-docs` to prepare; short-circuits with Minimal Fill Plan when readiness is too low.

**Phase 1**: Runs unified sequence `align-planning` (full) -> `assess-docs`, then output-driven follow-ups (`align-architecture`, `run-repair-loop`, `design-solution`, `analyze-requirements`) when findings indicate need. Aggregates outputs into a single cycle report with Recommended Next Tasks.
...
- `assess-docs` — doc evidence assessment
- `discover-docs-norms` — establish artifact paths (Phase 0.5)
