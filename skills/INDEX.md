...
| [auto-iterate](./auto-iterate/SKILL.md) | automation, meta-skill, workflow | `1.0.1` | stable | 单步治理执行器——读取 plan-next 路由输出，执行最高优先级动作，发出继续信号以支持 /loop 全自动推进。. |
| [plan-next](./plan-next/SKILL.md) | automation, meta-skill, workflow | `11.0.0` | mature | 盘点治理输入源并输出下一步技能路由——只读，永不执行下游；三步法扫/诊/荐：诊步骤逐目标遍历目标链（战略目标→路线图→需求→设计→任务→完成），找首个未就绪层级作为路由目标；深度优先，多目标按优先级顺序处理。完成判定只看任务 status 字段。. |
