...
| [plan-next](./plan-next/SKILL.md) | automation, meta-skill, workflow | `9.3.0` | mature | 盘点治理输入源并输出下一步技能路由——只读，永不执行下游；自动化由外层编排器 + loop 组合驱动。三步法 "扫/诊/荐"：诊步骤识别状态、发现缺口；荐步骤分层决策、生成路由。步骤 0 读规范路径；步骤 2.1.3 S5 任务×代码交叉判执行健康；步骤 2.2 内置当期层物理扫描分支；深度优先。. |
