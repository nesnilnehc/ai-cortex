...
| [plan-next](./plan-next/SKILL.md) | automation, meta-skill, workflow | `9.2.2` | mature | 盘点治理输入源并输出下一步技能路由——read-only，永不执行下游；自动化由外层 orchestrator + loop 组合驱动。三步法 "扫/诊/荐" + Step 0 Norms Resolution 读 path_pattern 覆盖；Step 2.1.3 按物理信号扫 Now tier 下游；Step 2.1.3 S5 任务×代码双信号交叉判执行健康；Now tier 深度优先。. |
