...
- **Pillar** = a direction of sustained investment (it does not get "finished"; an indispensable structural element, such as "reliable operation")
- **Goal** = a concrete result to reach in that direction (you can tell whether it has been met, such as "rollback rate below 5%")

A pillar is indispensable — drop one and the strategy has a structural gap. Goals change from stage to stage; pillars stay stable.
...
- [ ] **Pillars are indispensable**: Would dropping any one pillar leave the strategy with a structural gap? If not, that pillar is probably a goal or an initiative rather than a pillar.
- [ ] **Pillars cannot be completed**: Does each pillar describe a sustained direction rather than a result that can be judged as achieved?
