...
- [ ] 技能提供明确的 prioritization 框架或方法（如 MoSCoW、RICE、价值/成本矩阵、或与 project-cognitive-loop 兼容的排序流程）
- [ ] 技能已注册到 `skills/INDEX.md` 与 `manifest.json`，通过 `verify-registry.mjs` 校验
- [ ] 可与 `capture-work-items`、`analyze-requirements`、`project-cognitive-loop` 等形成清晰 handoff

## Notes

- 可与 `capture-work-items`（产出 captured 条目）、`execution-alignment`（目标对齐）衔接
