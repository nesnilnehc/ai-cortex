...
PERTURBATIONS = [
    ...
    code(SCENARIOS, SCENARIOS, "accept a citation that does not resolve",
     '        failed.add("TDES-025")\n',
     "        pass\n"),
    code(RULES, RULES_TEST, "derive every item as ready",
     "        ready = bool(owed) and all(fields.get(name) for name in owed)\n",
     "        ready = True\n"),
    code(RULES, RULES_TEST, "derive every item as provisional",
     "        ready = bool(owed) and all(fields.get(name) for name in owed)\n",
     "        ready = False\n"),
    code(RULES, RULES_TEST, "count an activation row present but empty as complete",
     "        ready = bool(owed) and all(fields.get(name) for name in owed)\n",
     "        ready = bool(owed) and all(name in fields for name in owed)\n"),
